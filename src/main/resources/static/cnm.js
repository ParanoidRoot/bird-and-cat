...
var texture_name_to_texture = {
	"eye1.png": init_texture("./model/eye1.png"),
	"mouth1.png": init_texture("./model/mouth1.png"),
	"pikagen.png": init_texture("./model/pikagen.png")
}
...
var bilibili_texture = init_texture("./model/Char_Patrick.png")
...
var bilibili2_texture = init_texture("./model/Char_Patrick.png")
var piTemp;
drawbilibili2()
...
function bilibili_convertData() {
	...
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(bilibili_textpo), gl.STATIC_DRAW);
	...
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(bilibili_normals), gl.STATIC_DRAW);

	// 绑定材质的纹理
	gl.bindBuffer(gl.ARRAY_BUFFER, bilibili_obj.texture_buffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(bilibili_textures), gl.STATIC_DRAW);
}
...
function bilibili2_convertData() {
	...
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(bilibili2_normals), gl.STATIC_DRAW);

	// 绑定材质的纹理
	gl.bindBuffer(gl.ARRAY_BUFFER, bilibili2_obj.texture_buffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(bilibili2_textures), gl.STATIC_DRAW);
}
...
function light_ball_convertData() {
	...
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(light_ball_vertices), gl.STATIC_DRAW);

	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, light_ball_obj.indexBuffer);
	gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(light_ball_faces), gl.STATIC_DRAW);

	gl.bindBuffer(gl.ARRAY_BUFFER, light_ball_obj.colorBuffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(light_ball_colors), gl.STATIC_DRAW);

	gl.bindBuffer(gl.ARRAY_BUFFER, light_ball_obj.normalBuffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(light_ball_normals), gl.STATIC_DRAW);
}


function test_cube_convertData() {
	gl.bindBuffer(gl.ARRAY_BUFFER, test_cube_obj.vertexBuffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(test_cube_vertices), gl.STATIC_DRAW);

	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, test_cube_obj.indexBuffer);
	gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(test_cube_faces), gl.STATIC_DRAW);

	gl.bindBuffer(gl.ARRAY_BUFFER, test_cube_obj.colorBuffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(test_cube_colors), gl.STATIC_DRAW);

	gl.bindBuffer(gl.ARRAY_BUFFER, test_cube_obj.normalBuffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(test_cube_normals), gl.STATIC_DRAW);
}
...
function mousewheel(e) {
	if (e.wheelDelta) { //判断浏览器IE，谷歌滑轮事件
		if (e.wheelDelta > 0) { //当滑轮向上滚动时
			bilibili_fieldOfViewRadians = degToRad(radToDeg(bilibili_fieldOfViewRadians) < 150 ? radToDeg(bilibili_fieldOfViewRadians) + 1 : radToDeg(bilibili_fieldOfViewRadians));
		}
		if (e.wheelDelta < 0) { //当滑轮向下滚动时
			bilibili_fieldOfViewRadians = degToRad(radToDeg(bilibili_fieldOfViewRadians) > 30 ? radToDeg(bilibili_fieldOfViewRadians) - 1 : radToDeg(bilibili_fieldOfViewRadians));
		}
...
function onKeyDown(event) {
	...
	//W
	if (event.keyCode == 87) {
		bilibili_translation[0] = bilibili_translation[0] + 10 * bilibili_directZ[0];
		bilibili_translation[1] = bilibili_translation[1] + 10 * bilibili_directZ[1];
		bilibili_translation[2] = bilibili_translation[2] + 10 * bilibili_directZ[2];
	}
	//S
	if (event.keyCode == 83) {
		bilibili_translation[0] = bilibili_translation[0] - 10 * bilibili_directZ[0];
	...
	//UP
	if (event.keyCode == 38) {
		bilibili2_translation[0] = bilibili2_translation[0] + 10 * bilibili2_directZ[0];
		bilibili2_translation[1] = bilibili2_translation[1] + 10 * bilibili2_directZ[1];
		bilibili2_translation[2] = bilibili2_translation[2] + 10 * bilibili2_directZ[2];
	}
	//DOWN
	if (event.keyCode == 40) {
...
function drawSceneIndex() {
	...
	changeMatrix = m4.translation(light_ball_center[0], light_ball_center[1], light_ball_center[2])
	var T = m4.translation(light_ball_translation[0], light_ball_translation[1], light_ball_translation[2]);
	...
	var S = m4.scaling(light_ball_scale[0], light_ball_scale[1], light_ball_scale[2]);
	unchangeMatrix = m4.translation(-light_ball_center[0], -light_ball_center[1], -light_ball_center[2])
	...
	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, light_ball_obj.indexBuffer);
	// // // 绘制
	gl.drawElements(gl.TRIANGLES, light_ball_faces.length, gl.UNSIGNED_SHORT, 0);
...
function drawbilibili2() {
	...
	// bilibili2_vertices = all_multi_by(temp.vertices, 2)
	// bilibili2_faces = temp.v_indexes
	// bilibili2_colors = temp.colors
	// bilibili2_normals = temp.normals
	// bilibili2_textures = temp.textures
...
{
	bilibili2_vertices = all_multi_by(make_one_dimension(piTemp[index].vertices), 2)
...
function get_texture_for_obj(_obj, prefix="./model/") {
